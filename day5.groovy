...
*/
debug = true
// input = 'lrbajafxwwnxvbei'

vowels = ['a','e','i','o','u']
def hasThreeVowels(def str) {
	def count = 0
	// def vowels = allVowels.collect()
	str.each {
	...
	if (debug) print "\tVowel count is: $count"
	count >= 3
}

def hasDoubleChar(def str) {
	def hasDouble = false
	...
	if (debug) print "\tHas Double is: $hasDouble"
	hasDouble
}
naughtyChars = ['ab', 'cd', 'pq', 'xy']
def hasNaughtyChars(def str) {
	def isNaughty = false
	naughtyChars.each {
		if(str.contains(it)){
			isNaughty = true
	...
	if (debug) print "\tIs Naughty is: $isNaughty"
	isNaughty
}

def checkRules(def input) {
	def goodCount = 0
	input.eachLine {
		print "Testing String: ${it}"
		if(hasThreeVowels(it) && hasDoubleChar(it) && !hasNaughtyChars(it)) {
			goodCount++
		}
		print '\n'
	}
